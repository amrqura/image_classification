...
In this script , I will write tensorflow code that will train convolutional neural 
network to train a binary image classification

In our dataset , we have two types labels:
cat(0), dogs(1)
we will train the convolutional neural network to predict the probability whether the 
given image is dog , so the probability of 1.0 means that the network is 100 % sure
that the image is dog , while 0 means that it is completely certain that it is cat. 
'''
...
def create_resized_images(input_folder, output_folder, select_files=None, out_xy=(img_size, img_size)):
    """
    input_folder: <string>, folder path 
    output_folder: <string>, output folder path
    select_files: <list> only select files from the input folder
    out_xy: (height, width) of resize images
    """
    ...
    for infile in files:
        ...
        if ext.lower() in [".jpg", ".jpeg"]:
            ...
            if not resized_im.mode == "RGB":   # "RGB" is a 8-bit 3 layered format
...
def load():
    ...
    valid_list: list of validation dataset imag names.
...
def prepare_dataset(folder_name,num_images,is_testing_dataset):
    '''
    This function prepare the dataet in format that is understandable
...
print 'format the dataset to the shape that is expected by neueral network'
...
def save_ouput(path):
  ...
  num_list=range(1,len(dog_testing_prediction)+1)
  matrix=np.column_stack((num_list,dog_testing_prediction))
